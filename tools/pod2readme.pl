#!/usr/bin/env perl
# Converts lib/DBIx/Wizard.pod to README.md with title-cased headers and proper structure.
use strict;
use warnings;
use Pod::Markdown;
...
my $pod_file = $ARGV[0] || 'lib/DBIx/Wizard.pod';
...
# Transform headers
my %acronyms = map { lc($_) => $_ } qw(SQL DBI DBIx CTEs CTE MySQL PostgreSQL SQLite ANSI);
my %methods = map { lc($_) => 1 } qw(
  ...
  join returning columns limit offset compare any all
  find having inflate cursor count dbh
